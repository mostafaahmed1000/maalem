...
@section('page_subtitle', 'List of consultation inquiry submissions')
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr style="text-align: left; border-bottom: 2px solid var(--border);">
                    ...
                    <th style="padding: 1.2rem; color: var(--text-muted); font-size: 0.85rem; text-transform: uppercase;">Mobile</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid var(--border); transition: background 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
                    ...
                    <td style="padding: 1.2rem;">
                        <span class="badge {{ $item->school_status == 'New' ? 'badge-new' : ($item->school_status == 'Established' ? 'badge-established' : 'badge-default') }}">
                            {{ $item->school_status ?? 'N/A' }}
                        </span>
                        @if($item->school_status == 'Established' && $item->established_years)
                            <div style="font-size: 0.8rem; color: var(--text-muted); margin-top: 4px;">{{ $item->established_years }} Years</div>
                        @endif
                    </td>
                    <td style="padding: 1.2rem;">
                    ...
                    <td style="padding: 1.2rem;">{{ $item->contact_mobile }}</td>
                    <td style="padding: 1.2rem; color: var(--text-muted);">{{ $item->created_at->format('M d, Y') }}</td>
                    <td style="padding: 1.2rem; text-align: right;">
                        <a href="{{ route('admin.submissions.show', ['type' => 'consultation', 'id' => $item->id]) }}" class="btn-view" style="color: var(--primary); font-size: 1.1rem; text-decoration: none;">
